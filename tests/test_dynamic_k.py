...
def test_dynamic_k_cap_follows_accept_ema(device):
    ...
    # Step 1: update_state jumps the EMA to 3 (>= release) before
    # drafting, so the row is uncapped and drafts the full chain.
    ...
    # Row recycled: EMA reset => standalone propose is re-capped.
    d.reset_rows([0])
    lens = torch.tensor([nts], dtype=torch.int32, device=device)
    _, nv_t = d.propose(lens, buf)
    assert nv_t[0].item() == 1
    # Next update_state relearns instantly (EMA jumps, released).
    nv, nts = _step(d, buf, nts, 4)
    assert nv > 3
